...
import { JwtAuthService } from 'src/2faJwt/jwt/JwtAuth.service';
import { userDto, userParitalDto } from 'src/User/User.dto';
import { UserService } from 'src/User/User.service';
import { authenticator } from 'otplib';
import { toDataURL } from 'qrcode';
import { Jwt2faAuthService } from 'src/2faJwt/2fa/2faAuth.service';
...
export class AuthService {
  ...
  async checkUserExist(user: userDto): Promise<userParitalDto> {
    let getUser: userParitalDto = await this.userService.getPartialUser(
      user.login,
    );
    if (!getUser) getUser = await this.userService.registerUser(user);
    return { ...getUser };
  }

  redirectProfile(res: Response) {
    res.redirect(`http://${this.configService.get('FRONTEND_IP')}/profile`);
  }
  ...
  async authenticateUser(user: userParitalDto, res: Response) {
    if (user.isFirst) res.cookie('isFirst', true);
    ...
    if (is2faEnabled) {
      ...
      return res.redirect(
        `http://${this.configService.get('FRONTEND_IP')}/?_2fa=true`,
      );
    }
  ...
  async is2faCodeValid(user: userParitalDto, code: string) {
    ...
    if (!isValid) return { err: 'Wrong authentication code' };
    return { data: true };
  }
