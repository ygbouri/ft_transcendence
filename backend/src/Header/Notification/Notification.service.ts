...
import { Notification, notifStatus } from './entity/Notification.entity';
...
export class NotificationService {
  ...
  async getNotifId(from: string, to: string) {
    const notif: Notification = await this.notifRepository
      ...
      .where(
        `notifications.from = '${from}' AND notifications.to = '${to}' AND notifications.status = '${notifStatus.SENT}'`,
      )
      .getOne();
    return notif;
  }
  ...
  async getNotifs(login: string) {
    ...
    const notifList = await Promise.all(
      notifs.map(async (notif) => {
        const userInfo: opponentDto = await this.userService.getOpponent(
          notif.from,
        );
        return {
          notifId: notif.id,
