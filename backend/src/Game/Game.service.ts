...
import { userAchievements } from 'src/User/entity/Stats.entity';
import { DBrank, DBstats } from 'src/User/User.dto';
...
export class GameService {
  ...
  async getRecentMatchesWon(player: string) {
    const matches = await this.gameRepository.query(
      `select games."playerOne", games."playerTwo", games."playerOneScore", games."playerTwoScore", games.date from games where (games."playerOne" = '${player}' or games."playerTwo" = '${player}') order by games.date DESC limit 20`,
    );
    if (matches.length >= 10) {
      ...
      if (!loss && matches.length === 20) return 20;
      if (!loss || (loss && loss.idx >= 10)) return 10;
    }
    return 0;
  }

  async updateStats(player: string, gameType: string, winner: boolean) {
    const stats: DBstats = await this.userService.getStats(player);
    stats.numGames++;
    stats.gamesWon += winner ? 1 : 0;
    if (gameType === 'Ranked')
      stats.GP += winner ? 50 : stats.GP >= 25 ? -25 : 0;
    const matches = Math.round(
      0.04 * ((stats.XP / 1000) ^ 3) +
        0.8 * ((stats.XP / 1000) ^ 2) +
        2 * (stats.XP / 1000),
    );
    ...
    const rankList: DBrank[] = await this.userService.getRank();
    const toBeUpdated: DBrank[] = [];
    ...
    toBeUpdated.forEach((rank) => {
      if (rank.id === stats.id) stats.rank = rank.rank;
      this.userService.updateRank(rank.id, rank.rank);
    });

    if (
      !stats.achievement.includes(userAchievements.FIRSTWIN) &&
      winner &&
      gameType === 'Ranked'
    )
    ...
    if (
      stats.XP >= 5000 &&
      !stats.achievement.includes(userAchievements.LEVEL5)
    )
      stats.achievement.push(userAchievements.LEVEL5);
    if (
      stats.GP >= 1600 &&
      !stats.achievement.includes(userAchievements.GOLDTIER) &&
      winner
    )
      stats.achievement.push(userAchievements.GOLDTIER);
    if (
      stats.rank === 1 &&
      !stats.achievement.includes(userAchievements.FIRSTPLACE) &&
      winner
    )
  ...
  async insertMatches(gameResult: gameDto) {
    ...
    const winner: string =
      game.playerOneScore > game.playerTwoScore
        ...
        : game.playerTwo;
    const loser: string =
      winner === game.playerOne ? game.playerTwo : game.playerOne;
  ...
  async getMatches(login: string) {
    const history: Game[] = await this.gameRepository
      ...
      .select([
        'games.playerOne',
        'games.playerTwo',
        'games.playerOneScore',
        'games.playerTwoScore',
        'games.date',
      ])
      .where(`games.playerOne = :login OR games.playerTwo = :login`, {
        login: login,
      })
      .orderBy('games.date', 'DESC')
      .getMany();
    if (!history.length) return { data: history };
    const matches = await Promise.all(
      history.map(async (match) => {
        const opponentLogin: string =
          match.playerOne === login ? match.playerTwo : match.playerOne;
        const opponent: opponentDto = await this.userService.getOpponent(
          opponentLogin,
        );
