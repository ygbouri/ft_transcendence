...
export class Game {
  ...
  public startGame(socket: Server, games: allGames) {
    ...
    if (this._Ball.collision(paddle)) {
      if (
        (this._Ball.getBallY() <
          paddle.getPaddleY() + GameControlers.PaddleH / 3 &&
      ...
      if (this._Ball.getBallSpeed() < 4.4) this._Ball.increaseSpeed();
    }
    ...
    if (
      this._PlayerLeft.getscore() === GameControlers.MaxScore ||
      this._PlayerRight.getscore() === GameControlers.MaxScore
    )
  ...
  public EndGame(socket: Server, games: allGames) {
    ...
    games.FriendsLobby = games.FriendsLobby.filter(
      (game) => game.idGame !== this._ID,
    );
    const gameResult: gameDto = {
      playerOne: this._PlayerLeft.getlogin(),
      playerTwo: this._PlayerRight.getlogin(),
      playerOneScore: this._PlayerLeft.getscore(),
      playerTwoScore: this._PlayerRight.getscore(),
  ...
  public resumeGame(client: Socket, login: string, server: Server) {
    if (
      login === this._PlayerLeft.getlogin() &&
      client.id !== this._PlayerLeft.getsocket().id
    ) {
      ...
      login === this._PlayerRight.getlogin() &&
      client.id !== this._PlayerRight.getsocket().id
    ) {
