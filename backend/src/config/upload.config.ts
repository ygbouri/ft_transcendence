...
export const uploadUsersConfig: MulterOptions = {
  ...
  fileFilter: (req, file, callback) => {
    if (!validMimeTypes.includes(file.mimetype)) callback(null, false);
...
export const isFileValid = async (uploadPath: string, name: string) => {
  ...
  const isValid: boolean =
    validFileExtensions.includes(fileType.ext) &&
    validMimeTypes.includes(fileType.mime);
  if (!isValid) {
...
export const deleteAvatar = async (uploadPath: string, name: string) => {
  if (name) fs.unlink(`./uploads/${uploadPath}/${name}`, (err) => {});
};
...
export const resizeAvatar = async (uploadPath: string, name: string) => {
  ...
  image
    .resize(220, 220)
    .write(`./uploads/${uploadPath}/${resizeName}x220${ext}`);
