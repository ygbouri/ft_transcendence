...
import { notifStatus } from 'src/Header/Notification/entity/Notification.entity';
import { NotificationService } from 'src/Header/Notification/Notification.service';
import { UserService } from 'src/User/User.service';
import { Repository } from 'typeorm';
import { friendST } from './Friendship.dto';
import { Friendship, userRelation } from './entity/Friendship.entity';

@Injectable()
export class Service_of_friendship {
  constructor(
  ...
  async getRelation(user: string, friend: string) {
    const friendship: Friendship = await this.friendshipRepository
      ...
      .where(
        '(friendships.user = :user AND friendships.friend = :friend) OR (friendships.user = :friend AND friendships.friend = :user)',
        { user: user, friend: friend },
      )
      .getOne();
    if (!friendship) return userRelation.NONE;
    if (
  ...
  async getChatRelation(user: string, friend: string) {
    const friendship: Friendship = await this.friendshipRepository
      ...
      .where('friendships.user = :user AND friendships.friend = :friend', {
        user: user,
        friend: friend,
      })
      .getOne();
  ...
  async getOnlineFriends(login: string) {
    const friends: Friendship[] = await this.friendshipRepository
      ...
      .where(
        'friendships.relation = :relation AND (friendships.user = :login OR friendships.friend = :login)',
        { relation: userRelation.FRIEND, login: login },
      )
      .getMany();
    if (!friends.length) return { data: friends };
    const friendList = [];
    await Promise.all(
      friends.map(async (friend) => {
        const friendLogin = friend.user === login ? friend.friend : friend.user;
        const friendInfo: friendST = await this.userService.getFriend(
          friendLogin,
        );
        if (friendInfo.status === 'Online') friendList.push(friendInfo);
      }),
    );
  ...
  async getRequests(login: string) {
    const requests: Friendship[] = await this.friendshipRepository
      ...
      .where(
        'friendships.relation = :relation AND friendships.friend = :login',
        { relation: userRelation.REQUESTED, login: login },
      )
      .getMany();
    if (!requests.length) return { data: requests };
    const requestsList = await Promise.all(
      requests.map(async (friend) => {
        const requestInfo: friendST = await this.userService.getFriend(
          friend.user,
        );
        return requestInfo;
      }),
  ...
  async getPending(login: string) {
    const pending: Friendship[] = await this.friendshipRepository
      ...
      .where('friendships.relation = :relation AND friendships.user = :login', {
        relation: userRelation.REQUESTED,
        login: login,
      })
      .getMany();
    if (!pending.length) return { data: pending };
    const pendingList = await Promise.all(
      pending.map(async (friend) => {
        const pendingInfo: friendST = await this.userService.getFriend(
          friend.friend,
        );
        return pendingInfo;
      }),
  ...
  async getBlocked(login: string) {
    const blocked: Friendship[] = await this.friendshipRepository
      ...
      .getMany();
    if (!blocked.length) return { data: blocked };
    const blockedList = await Promise.all(
      blocked.map(async (friend) => {
        const blockedInfo: friendST = await this.userService.getFriend(
          friend.friend,
        );
        return blockedInfo;
      }),
  ...
  async addFriend(user: string, friend: string) {
    const exist: Friendship = await this.friendshipRepository
      ...
      .getOne();
    if (exist) return { err: 'Relation already exist' };
  ...
  async unfriend(user: string, friend: string) {
    this.friendshipRepository
      ...
      .delete()
      .where(
        '((friendships.user = :user AND friendships.friend = :friend) OR (friendships.user = :friend AND friendships.friend = :user)) AND friendships.relation = :relation',
        { user: user, friend: friend, relation: userRelation.FRIEND },
      )
      .execute();
  }
  ...
  async acceptRequest(user: string, friend: string) {
    await this.friendshipRepository
      ...
      .where(
        'friendships.user = :friend AND friendships.friend = :user AND friendships.relation = :relation',
        { user: user, friend: friend, relation: userRelation.REQUESTED },
      )
      .execute();
  ...
  async cancelRequest(user: string, friend: string) {
    await this.friendshipRepository
      ...
      .delete()
      .where(
        'friendships.user = :user AND friendships.friend = :friend AND friendships.relation = :relation',
        { user: user, friend: friend, relation: userRelation.REQUESTED },
      )
      .execute();
    return true;
  ...
  async blockUser(user: string, friend: string) {
    await this.friendshipRepository
      ...
      .delete()
      .where(
        '(friendships.user = :user OR friendships.user = :friend) AND (friendships.friend = :friend OR friendships.friend = :user)',
        { user: user, friend: friend },
      )
      .execute();
  ...
  async unblock(user: string, friend: string) {
    await this.friendshipRepository
      ...
      .where(
        'friendships.user = :user AND friendships.friend = :friend AND friendships.relation = :relation',
        { user: user, friend: friend, relation: userRelation.BLOCKED },
      )
      .execute();
