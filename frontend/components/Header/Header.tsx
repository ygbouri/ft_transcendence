...
const Notif_item: React.FC<NotificationType> = (props) => {
  ...
  const acceptHandler = async () => {
    if (props.type.includes("Request")) {
      ...
      socket_game.emit(
        "joinGameFriend",
        { login: owner, accept: props.gameId },
        (data: string) => {
          if (data.length > 4) {
            socket_notif.emit("updateGame", {
  ...
  const refuseHandler = async () => {
    if (props.type.includes("Request")) {
      ...
      socket_notif.emit(
        "updateGame",
        { notifId: props.notifId, status: "Refused" },
        () => {
          props.refresh();
      ...
      <div className={classes.notif}>
        <div className={classes.notifContent} onClick={clickHandler}>
          <div className={classes.avatar_notif}>
            <img src={avatarPath} alt={props.login} />
          </div>
            ...
            {dropDown && (
              <div className={classes.DropDown}>
                <div className={classes.LogOut}>
                  ...
                  <div onClick={toggleSettingHandler}>Settings</div>
                </div>
                <div className={classes.LogOut}>
                  <div className={classes.Icon}>
                    <FiLogOut />
...
const Header: React.FC<{ setPos: (page: string) => void }> = (props) => {
  ...
  const router = useRouter();
  const searchHandler: FormEventHandler = (e) => {
  ...
  useEffect(() => {
    ...
    socket_game.on("gameStartedSoon", (props) => {
      if (props.check === true) {
        if (!router.asPath.includes("game/lobby?gameID="))
          router.push("/game/lobby?gameID=" + props.id);
      }
    });
    return () => {
      ...
      <div className={classes.tmpctn}>
        <div className={classes.inputContainer}>
          <div className={classes.searchIcon}>
            <FiSearch size="18px" color="white" />
          </div>
