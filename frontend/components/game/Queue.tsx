...
const CardPlayer: React.FC<{ Loading: boolean; login: string }> = ({
  Loading,
  login,
}) => {
  ...
  useEffect(() => {
    if (login) fetchDATA(setUserInfo, router, `user/info/${login}`);
    setIsMounted(true);
    return () => {
          ...
          {(Loading && (
            ...
            <>
              <div className={classes.items}>
                ...
                <div className={classes.NAme}>{userInfo.login}</div>
              </div>
            </>
          )}
        </div>
...
const Queue: React.FC<{ cancel: (t: string) => void }> = ({ cancel }) => {
  const router = useRouter();
